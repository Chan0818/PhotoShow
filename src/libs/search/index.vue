<template>
    ...
    class="relative p-0.5 rounded-xl border-white duration-500 
    group hover:bg-red-100">
      <div >
          <!-- 搜索图标 -->
          <m-svg-icon class="absolute left-2 w-1.5 h-1.5
          top-[50%] translate-y-[-50%]"
          name="search" color="#707070"></m-svg-icon>
          <!-- 输入框 -->
          <input type="text"
          ...
          v-show="inputValue"
          @click="onClearClick"
          class="absolute right-9 w-1.5 h-1.5 cursor-pointer
          top-[50%] translate-y-[-50%] duration-300"
          name="input-delete"
          ></m-svg-icon>
          <!-- 分割线 -->
          <div class="absolute right-[62px] w-[1px] h-1.5 top-[50%]
          translate-y-[-50%] duration-500 bg-zinc-200 opacity-0 group-hover:opacity-100"></div>
          <!-- TODO:搜索按钮:通用组件 -->
          <m-button class="absolute right-1 top-[50%] translate-y-[-50%] rounded-xl opacity-0 group-hover:opacity-100 duration-500"
          icon="search"
          iconColor="#ffffff"
